...
公开页面统一加载 `/fresh-theme.css`：以低发光感的暖中性纸张色为大面积背景，表面只略微
提亮，正文采用咖啡叶深绿炭灰，并用低饱和陶土红和焦糖色点缀。主题保持深字浅底，正文
对比度约 8:1；表单、长内容区和应用大面板避免纯白、纯黑及大幅明暗跳变。该样式只用于
公开层；Legacy `private/` 与 `private-src/` 不加载它。
